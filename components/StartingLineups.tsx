...
function rosterMatch(
  ...
  return players.find((player) => {
    ...
    return (
      playerFullName(player) === `${statPlayer.first_name} ${statPlayer.last_name}`.toLowerCase()
    );
  });
}
...
function TeamLineup({ team, players }: { team: GameDisplay['homeTeam']; players: LineupPlayer[] }) {
  return (
    <div>
          ...
          {players.map((player) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {player.position && (
                  <div className="text-[10px] font-semibold text-ui-muted">{player.position}</div>
                )}
                {player.statLine && (
